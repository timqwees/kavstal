      ...
      function render(cat) {
        ...
        items.forEach(function (p) {
          ...
          h += '<div class="border border-gray-200 hover:border-gray-300 transition-all duration-200 rounded-xl p-3 flex flex-col bg-white" itemscope itemtype="https:' + '//schema.org/Product">' +
            ...
            '<button type="button" class="add-to-fav-btn w-7 h-7 rounded-md border border-gray-200 flex items-center justify-center shrink-0 hover:border-gray-400 hover:bg-gray-50 transition-colors" data-pid="' + (p.pid || p.id) + '" title="В избранное">' +
            ...
            '</button>' +
            '</div>' +
            '<a href="' + p.url + '" class="flex items-center justify-center h-[120px] mb-3 rounded-lg overflow-hidden bg-gray-50" style="aspect-ratio:4/3" aria-label="' + p.name.replace(/"/g, '&quot;') + '">' +
            (p.image ? '<img src="' + p.image + '" alt="' + p.name.replace(/"/g, '&quot;') + '" class="max-h-full max-w-full object-contain" loading="lazy">' : '<i class="fas fa-cube text-4xl text-gray-300"></i>') +
            '</a>' +
          ...
          h += '<div class="mt-auto">' +
            ...
            '<span class="text-[11px] font-medium ' + (p.inStock ? 'text-amber-600' : 'text-gray-400') + '">' + (p.inStock ? 'Уточняйте наличие' : 'Под заказ') + '</span>' +
            '</div>' +
            ...
            '</div>' +
            '</div>' +
            '<button type="button" class="add-to-cart-btn w-8 h-7 rounded-lg bg-red-500 hover:bg-red-500 active:bg-red-500 text-white flex items-center justify-center shrink-0 transition-colors" data-pid="' + (p.pid || p.id) + '" data-unit="' + fu + '" title="В заявку">' +
