...
<body class="bg-gray-50">
    ...
    <main class="max-w-7xl mx-auto px-4 pb-16">
        ...
        <div id="orders-empty" class="hidden text-center py-20">
            ...
            <a href="/market"
                class="inline-block bg-red-500 text-white px-6 py-2.5 rounded-lg text-sm font-medium hover:bg-red-500 transition">Перейти
                в каталог</a>
        </div>

        <!-- Loading -->
        <div id="orders-loading" class="text-center py-20">
            <div class="inline-block w-8 h-8 border-3 border-red-300 border-t-red-500 rounded-full animate-spin"></div>
            ...
            fetch('/api/orders/list').then(function (r) { return r.json(); }).then(function (d) {
                ...
                orders.forEach(function (order) {
                    ...
                    card.innerHTML =
                        ...
                        '<div class="text-lg font-bold text-red-500">' + parseFloat(order.total).toLocaleString('ru-RU') + ' ₽</div>' +
                        '<a href="/order/' + order.id + '/pdf" class="text-xs text-gray-400 hover:text-red-500 transition inline-flex items-center gap-1" target="_blank">' +
