...
$deliveryMethods = [
    'pickup' => ['label' => 'Самовывоз', 'desc' => 'Проектируемый проезд № 8342, вл. 5, Хованская промзона', 'price' => 0],
...
<head>
    ...
    <link rel="preload" href="https://cdn.jsdelivr.net/npm/@suggestions/da-suggestions@2.0.6/dist/css/suggestions.css"
        as="style" onload="this.onload=null;this.rel='stylesheet'">
    <noscript>
        <link rel="stylesheet"
            href="https://cdn.jsdelivr.net/npm/@suggestions/da-suggestions@2.0.6/dist/css/suggestions.css">
    </noscript>
    <style>
    ...
    <script src="https://cdn.jsdelivr.net/npm/@suggestions/da-suggestions@2.0.6/dist/js/jquery.suggestions.min.js"
        defer></script>
...
<body class="bg-gray-50">
    ...
    <main class="max-w-7xl mx-auto px-4 py-8">
        <div class="flex items-center gap-3 mb-8">
            ...
            <span class="text-sm text-gray-400 bg-gray-100 px-3 py-1 rounded-full"><?= $cartCount ?>
                <?= $cartCount === 1 ? 'товар' : ($cartCount > 1 && $cartCount < 5 ? 'товара' : 'товаров') ?></span>
        </div>

        <div class="grid grid-cols-1 lg:grid-cols-5 gap-8">
            <div class="lg:col-span-3 space-y-6">
                <form id="checkout-form" action="/checkout" method="POST" class="flex flex-col gap-6"
                    data-goal="checkout" data-ajax="false">
                    <!-- Контактные данные -->
                    <div class="bg-white rounded-xl shadow-sm border border-gray-100">
                        <div class="section-toggle flex items-center justify-between p-5" onclick="toggleSection(this)">
                            <h2 class="text-lg font-bold text-gray-900"><i
                                    class="fas fa-user text-red-500 mr-2"></i>Контактные данные</h2>
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-gray-100">
                        <div class="section-toggle flex items-center justify-between p-5" onclick="toggleSection(this)">
                            <h2 class="text-lg font-bold text-gray-900"><i
                                    class="fas fa-truck text-red-500 mr-2"></i>Способ доставки</h2>
                            <i class="fas fa-chevron-down text-gray-400 arrow"></i>
                        </div>
                        <div class="section-body px-5 pb-5 space-y-3">
                            <?php foreach ($deliveryMethods as $key => $dm): ?>
                                    <label
                                        class="option-card flex items-center gap-4 <?= $key === 'pickup' ? 'selected' : '' ?>">
                                        <input type="radio" name="delivery" value="<?= $key ?>" <?= $key === 'pickup' ? 'checked' : '' ?>>
                                        <div
                                            class="w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 <?= $key === 'pickup' ? 'border-red-500' : 'border-gray-300' ?>">
                                            <div class="w-2.5 h-2.5 rounded-full <?= $key === 'pickup' ? 'bg-red-500' : '' ?>">
                                            </div>
                                        </div>
                                        ...
                                        <div
                                            class="text-sm font-semibold whitespace-nowrap <?= $dm['price'] === 0 ? 'text-green-600' : 'text-gray-900' ?>">
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-gray-100" id="address-section">
                        <div class="section-toggle flex items-center justify-between p-5" onclick="toggleSection(this)">
                            <h2 class="text-lg font-bold text-gray-900"><i
                                    class="fas fa-map-marker-alt text-red-500 mr-2"></i>Адрес доставки</h2>
                            <i class="fas fa-chevron-down text-gray-400 arrow"></i>
                        </div>
                        <div class="section-body px-5 pb-5 space-y-4">
                            <div>
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-gray-100">
                        <div class="section-toggle flex items-center justify-between p-5" onclick="toggleSection(this)">
                            <h2 class="text-lg font-bold text-gray-900"><i
                                    class="fas fa-wallet text-red-500 mr-2"></i>Способ оплаты</h2>
                            <i class="fas fa-chevron-down text-gray-400 arrow"></i>
                        </div>
                        <div class="section-body px-5 pb-5 space-y-3">
                            <?php foreach ($paymentMethods as $key => $pm): ?>
                                    <label
                                        class="option-card flex items-center gap-4 <?= $key === 'transfer' ? 'selected' : '' ?>">
                                        <input type="radio" name="payment" value="<?= $key ?>" <?= $key === 'transfer' ? 'checked' : '' ?>>
                                        <div
                                            class="w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 <?= $key === 'transfer' ? 'border-red-500' : 'border-gray-300' ?>">
                                            <div
                                                class="w-2.5 h-2.5 rounded-full <?= $key === 'transfer' ? 'bg-red-500' : '' ?>">
                                            </div>
                                        </div>
                                        <div
                                            class="w-9 h-9 bg-gray-50 rounded-lg flex items-center justify-center flex-shrink-0">
                    ...
                    <div class="bg-gray-50 rounded-xl p-4 text-sm text-gray-600 border border-gray-100">
                        <p><i class="fas fa-info-circle text-red-500 mr-2"></i>Нажимая «Оформить заказ», вы соглашаетесь
                            на <a href="#" class="text-red-500 underline">обработку персональных данных</a>.</p>
                    </div>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 sticky"
                        ...
                        <?php foreach ($cartItems as $item): ?>
                                <div class="flex items-start gap-3 pb-3 border-b border-gray-100 last:border-0">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        ...
                                        <p class="text-xs text-gray-500"><?= number_format($item['quantity'], 2, ',', ' ') ?>
                    ...
                    <div class="border-t border-gray-200 pt-4 mt-3 space-y-1">
                        ...
                        <div class="flex justify-between items-center pt-2 border-t border-gray-200 mt-2">
                            ...
                            <span class="text-2xl font-bold text-red-500"><?= number_format($cartTotal, 2, ',', ' ') ?>
                                ₽</span>
                        </div>
                    </div>
                </div>
