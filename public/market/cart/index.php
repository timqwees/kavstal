    ...
    <div class="bg-white border-b border-gray-200">
        <div class="max-w-7xl mx-auto px-4 py-3">
            <nav class="flex items-center space-x-2 text-sm">
                <a href="/" class="text-gray-600 hover:text-red-500" aria-label="Главная"><i
    ...
    <main class="max-w-7xl mx-auto px-4 py-8">
        ...
        <?php if (empty($cartItems)): ?>
                <div class="bg-white rounded-2xl shadow-md p-12 text-center max-w-lg mx-auto">
                    ...
                    <a href="/market"
                        class="inline-flex items-center px-6 py-3 bg-red-500 text-white rounded-lg font-medium hover:bg-red-500 transition">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    <div class="lg:col-span-2 space-y-4">
                        <?php foreach ($cartItems as $item): ?>
                                <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 cart-item"
                                    ...
                                    <div class="flex items-center gap-4">
                                        ...
                                        <div class="flex-1 min-w-0">
                                            <a href="<?= htmlspecialchars($item['product_url']) ?>"
                                                class="text-sm font-medium text-gray-900 hover:text-red-500 transition line-clamp-2"><?= htmlspecialchars($item['product_name']) ?></a>
                                            <div class="flex flex-wrap gap-2 mt-1">
                                                <?php foreach ($item['specs'] as $specName => $specValue): ?>
                                                        <span
                                                            class="text-xs text-gray-500 bg-gray-50 px-2 py-0.5 rounded"><?= htmlspecialchars($specName) ?>:
                                                            <?= htmlspecialchars($specValue) ?></span>
                                                <?php endforeach; ?>
                                            </div>
                                        </div>
                                    ...
                                    <div class="flex items-center justify-between mt-4 pt-4 border-t border-gray-100">
                                        <div class="flex items-center gap-2">
                                            <button
                                                class="qty-btn qty-minus w-8 h-8 rounded-lg border border-gray-200 flex items-center justify-center text-gray-600 hover:bg-gray-100 transition text-sm"
                                                data-action="decrease">−</button>
                                            <input type="number"
                                                class="qty-input w-16 h-8 text-center border border-gray-200 rounded-lg text-sm font-medium"
                                                value="<?= (int) $item['quantity'] ?>" min="0" step="1"
                                                data-product-id="<?= htmlspecialchars($item['product_id']) ?>"
                                                data-unit="<?= htmlspecialchars($item['unit']) ?>">
                                            <button
                                                class="qty-btn qty-plus w-8 h-8 rounded-lg border border-gray-200 flex items-center justify-center text-gray-600 hover:bg-gray-100 transition text-sm"
                                                data-action="increase">+</button>
                                            <span class="text-xs text-gray-400 ml-1"><?= htmlspecialchars($item['unit']) ?></span>
                                        </div>
                                        <div class="flex items-center gap-4">
                                            <span
                                                class="text-base font-bold text-gray-900 item-subtotal"><?= number_format($item['subtotal'], 2, ',', ' ') ?>
                                                ₽</span>
                                            <button class="remove-item text-gray-400 hover:text-red-500 transition p-1"
                                                data-product-id="<?= htmlspecialchars($item['product_id']) ?>"
                                                data-unit="<?= htmlspecialchars($item['unit']) ?>">
                        ...
                        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 sticky top-28">
                            ...
                            <div class="space-y-3 mb-4">
                                <?php foreach ($cartItems as $item): ?>
                                        <div class="flex justify-between text-sm">
                                            <span
                                                class="text-gray-600 truncate max-w-[180px]"><?= htmlspecialchars($item['product_name']) ?></span>
                                            <span class="text-gray-900 font-medium"><?= number_format($item['subtotal'], 2, ',', ' ') ?>
                                                ₽</span>
                                        </div>
                                <?php endforeach; ?>
                            </div>
                            <div class="border-t border-gray-200 pt-4 mb-6">
                                <div class="flex justify-between items-center">
                                    ...
                                    <span
                                        class="text-2xl font-bold text-red-500 cart-total"><?= number_format($cartTotal, 2, ',', ' ') ?>
                                        ₽</span>
                                </div>
                            </div>
                            <a href="/checkout"
                                class="block w-full bg-red-500 text-white text-center py-3 rounded-lg font-medium hover:bg-red-500 transition">
                                <i class="fas fa-arrow-right mr-2"></i> Оформить заказ
                            </a>
                            <a href="/market"
                                class="block w-full text-center py-3 mt-2 text-sm text-gray-600 hover:text-red-500 transition">
