...
class Routes extends Network
{
    ...
    public static function addRoute(string $method, string $path, $callback): void
    {
        ...
        // Поддержка кастомных паттернов: {name:regex} например {path:.+}
                ...
                if (is_array($callback) && isset($callback[0], $callback[1])) {
                    ...
                    if ($isStatic) {
                        ...
                        if (method_exists($controllerClass, $action)) {
                            // Для POST запросов передаем $_POST данные как первый аргумент
                    ...
                    // Для POST запросов добавляем $_POST данные в именованные параметры
                    if ($method === 'POST' && !empty($_POST)) {
                        $named_params['post'] = (object) $_POST;
                    }
